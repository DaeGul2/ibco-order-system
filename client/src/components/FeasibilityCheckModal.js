...
import IngredientOrderFormModal from './IngredientOrderFormModal';
...
const FeasibilityCheckModal = ({ open, onClose }) => {
    ...
    const handleGenerateOrder = () => {
        const missing = groupByCase(3).map(row => ({
            id: row.ingredientId,
            name: row.ingredientName,
            quantityKg: Math.ceil(row.lackingKg * 10) / 10,
            cost: 0, // 필요시 cost fetch 추가
        }));
        setDefaultOrderItems(missing);
    ...
    return (
        <>
            ...
            <IngredientOrderFormModal
                ...
                defaultItems={defaultOrderItems}
            />
        </>
    );
};
